export const emailValidation = (dados)=>{
        if(dados === ''){
            ...
            if(!dados.includes('@')){
                ...
                if( (!dados.split('@')[1].includes('.')) || dados.split('@')[0].length === 0){
                    return {mensagem:'Erro na formatação do email', tipo: false}
                }else{
                    if(dados.split('@')[1].split('.')[0].length === 0){
                        return {mensagem:'Erro na formatação do email - domínio inválido', tipo:false}
                    }else{
                        if(dados.split('@')[1].split('.')[dados.split('@')[1].split('.').length -1].length=== 0){
                            return{mensagem:'Erro na formatação do email - email inválido', tipo:false}
                        }else{
